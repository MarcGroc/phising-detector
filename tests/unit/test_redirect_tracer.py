...
async def test_trace_redirects_happy_path_with_redirects(respx_mock: MockRouter):
    ...
    #3. ACT
    checker = RedirectCheck()
    result = await checker.run(AnyHttpUrl(initial_url))

    #4. ASSERTS
    assert result.check_name == "Redirect Check"
    assert result.details["chain_completed"] is True
    assert result.is_suspicious is True
    assert str(result.details["final_url"]) == final_url
...
async def test_trace_redirects_succeeds_on_retry(respx_mock: MockRouter):
    ...
    #2. ACT
    checker = RedirectCheck()
    result = await checker.run(AnyHttpUrl(retry_url))

    #3. ASSERTS
    assert result.is_suspicious is False
    assert result.details["chain_completed"] is True
    assert str(result.details["final_url"]) == retry_url
...
async def test_trace_redirects_fails_after_all_retries(mocker):
    ...
    #2. ACT
    checker = RedirectCheck()
    result = await checker.run(AnyHttpUrl(dead_url))

    # ASSERT
    assert result.is_suspicious is False
    assert result.details["chain_completed"] is False    # Must be False
    assert result.details["hops"] == 0
    assert str(result.details["final_url"]) == dead_url
